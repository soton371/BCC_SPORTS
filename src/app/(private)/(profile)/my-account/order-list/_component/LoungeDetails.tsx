...
const LoungeDetails = ({ orderId }: { orderId: string }) => {
  const { data: order, isLoading, isSuccess } = useGetLoungeOrderDetailsQuery(orderId);
  ...
  return (
    <>
      ...
      <div className='' hidden>
        <div ref={contentRef}>
          <VoucherDownload key={inData?.data?.id} order={inData?.data} />
        </div>
      </div>
    </>
  );
